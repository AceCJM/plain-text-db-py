...
class PlainDB:
    ...
    def insert(
        self, group_name: str, obj_name: str, obj_type: str, immutable: bool, value: str
    ):
        ...
        if obj_name in self.db[group_name]:
            if self.db[group_name][obj_name][1] == "true":
                print(
                    f"Cannot modify immutable object '{obj_name}' in group '{group_name}'."
                )
                return
    ...
    def delete(self, group_name: str, obj_name: str):
        ...
        if group_name in self.db and obj_name in self.db[group_name]:
            if self.db[group_name][obj_name][1] == "true":
                raise ValueError(
                    f"Cannot delete immutable object '{obj_name}' in group '{group_name}'."
                )
